...
class FindFile:
    def __init__(self,url):
        ...
        self.html = BeautifulSoup(html.text,'html5lib')
        self.tr = []
        for a in self.html.pre:
            if type(a).__name__ == 'Tag':
                self.tr.append((a.text,a['href'],a.next_sibling.strip()))
...
class SpiderDownload:
    def __init__(self,urls,localdir,exlude_dir=[""]):
        self.all_size = 0
    ...
    def _DoDownloadFile(self,data):
        ...
        try:
            ...
            parent = "https://archives.fedoraproject.org/pub/archive/fedora/linux/" + data.parent[data.parent.find('updates'):]

            parsed = urllib.parse.urlparse(parent)
            localdir = localdir + parsed.path[parsed.path.find("updates"):]
            ...
            if not os.path.exists(localdir+data.link):
                print("Downloading... " + str(round(self.download_size/1024/1024,2)) + "/" + str(round(self.all_size/1024/1024,2)) + " MB " + parent + data.link + "," + str(round(data.size/1024,2)) + "KB")
                html = requests.get(parent + data.link,headers=self.header)
                while html.status_code != 200:
                    time.sleep(1)
                    html = requests.get(parent + data.link,headers=self.header)
                ...
                print("Exsits "+ parent + data.link + " at " + localdir+data.link)
...
if __name__ == "__main__":
    exclude = ['Cloud','Container','Silverblue','Spins','armhfp','iso','aarch64','source','images','isolinux','EFI','debug','SRPMS']
    sd = SpiderDownload(["http://mirrors.kernel.org/fedora-buffet/archive/fedora/linux/updates/29/"],"E:\\Fedora29\\fedora\\",exclude)
    #st = int(time.time())
    #sd.GetAllSize(True)
